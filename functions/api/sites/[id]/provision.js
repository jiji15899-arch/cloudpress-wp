...
`.trim();
    ...
    console.error('[provision] 예외 발생:', e.message, e.stack);
    currentErrorMessage = e.message || '알 수 없는 오류';
    if (siteId && env?.DB) {
      await failSite(env.DB, siteId, currentProvisionStep || 'unknown', currentErrorMessage).catch(() => {});
    }
    return err('프로비저닝 오류: ' + currentErrorMessage, 500);
  }
}
