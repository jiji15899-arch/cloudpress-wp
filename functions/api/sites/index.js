...
async function dispatchToWorker(env, siteId, payload) {
  ...
  if (!workerUrl) {
    ...
    await env.DB.prepare(
      "UPDATE sites SET status='failed',error_message='Worker URL 미설정',updated_at=datetime('now') WHERE id=?"
  ...
  fetch(`${workerUrl}/api/provision-hosting`, {
    ...
    .then(async (res) => {
      ...
      if (!result.ok) {
        await env.DB.prepare(
          "UPDATE sites SET status='failed',error_message=?,updated_at=datetime('now') WHERE id=?"
            ...
            if (autoSslRow?.value === '1') {
              fetch(`${workerUrl}/api/setup-ssl`, {
                ...
                .then(async (r3) => {
                  ...
                  if (r3data.ok) {
                    await env.DB.prepare(
                      "UPDATE sites SET ssl_active=1,updated_at=datetime('now') WHERE id=?"
                    ).bind(siteId).run().catch(() => {});
                  }
                })
                .catch(() => {});
