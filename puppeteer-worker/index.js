...
const PROVIDERS = {
  ...
  infinityfree: {
    ...
    async provision(page, { email, password, siteName }) {
      ...
      const baseSlug = siteName.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 10) || 'cp';
      ...
      if (!usernameOk) {
        throw new Error('호스팅 계정 생성 폼을 찾을 수 없습니다.');
      }
      ...
      let cpanelUrl = 'https://cpanel.infinityfree.net';
  ...
  byethost: {
    ...
    async provision(page, { email, password, siteName }) {
      ...
      const baseSlug = siteName.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 12) || 'cp';
